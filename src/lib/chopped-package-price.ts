/**
 * Espelha public.compute_chopped_package_price no Postgres.
 * Usado só quando a RPC não está no schema cache do PostgREST (fallback).
 * Manter alinhado com supabase/functions/_shared/chopped-package-price.ts
 */
...
function kindMatches(rowKind: string | null | undefined, wantedKind: string): boolean {
  ...
  if (r === w) return true;
  // Bancos antigos: kind NULL/vazio era só seguidores (antes da coluna kind)
  if (w === "followers" && r === "") return true;
  return false;
}

/** PostgREST ainda não enxergou a função (projeto/cache diferente da SQL Editor). */
...
export function computeChoppedPackagePrice(
  ...
  // Vercel com VITE_FOLLOWERS_SERVICE_ID setado e pacotes sem service_id preenchido → lista vazia
